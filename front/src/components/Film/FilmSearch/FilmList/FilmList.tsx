...
const FilmList: React.FC<FilmListPropsType> = ({
  ...
  const filterFilms = useCallback((film: Film) =>
    (filter.name !== '' ? film.title.toLowerCase().includes(filter.name?.toLowerCase()) : true)
    && (filter.cinema !== '' ? cinemas?.find(cinema => cinema.id === filter.cinema)?.movieIds?.includes(film.id) : true)
    && (filter.genre !== '' ? film.genre.toLowerCase().includes(filter.genre?.toLowerCase()) : true), [filter, cinemas]);
